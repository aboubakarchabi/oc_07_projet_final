...
def request_prediction(model_uri, data):
    ...
    data_json = {"columns":["CODE_GENDER","CNT_CHILDREN", "AMT_INCOME_TOTAL", "AMT_CREDIT", "REGION_POPULATION_RELATIVE", "EXT_SOURCE_3", "CREDIT_TERM", "YEARS_EMPLOYED", "YEARS_REGISTRATION","YEARS_ID_PUBLISH"],"data": data}
...
def main():
    ...
    CODE_GENDER = st.number_input('client sex', min_value=0 , max_value=1)
    CNT_CHILDREN = st.number_input('Number of children',min_value=0, max_value=5, step=1)
    AMT_INCOME_TOTAL = st.number_input('Total income',min_value=25650.0, max_value=337500.0, step=10000.0)
    AMT_CREDIT = st.number_input('Mount of credit',min_value=45000, max_value=1485000, step=10000)
    REGION_POPULATION_RELATIVE = st.number_input('Population relative of the region',min_value=0.00029, max_value=0.072, step=0.02)
    EXT_SOURCE_3 = st.number_input('Exterieur source 3',min_value=0.0005,max_value=0.896,step=0.05)
    CREDIT_TERM = st.number_input('Credit terme',min_value=0.02, max_value=0.12,step= 0.05)
    YEARS_EMPLOYED = st.number_input('Years of employed',min_value=0.0,max_value=48.0,step=2.0)
    YEARS_REGISTRATION = st.number_input('Years of registration',min_value= 0.0,max_value=65.0,step=5.0)
    YEARS_ID_PUBLISH = st.number_input('Years of publish',min_value= 0.0, max_value=20.0, step=2.0)

    predict_btn = st.button('Predict')
    if predict_btn:
        data = [[CODE_GENDER,CNT_CHILDREN,AMT_INCOME_TOTAL,AMT_CREDIT,REGION_POPULATION_RELATIVE,EXT_SOURCE_3,CREDIT_TERM,
        YEARS_EMPLOYED,YEARS_REGISTRATION,YEARS_ID_PUBLISH]]
        print(data)

        pred = request_prediction(MLFLOW_URI, data)[0]
        if pred >0:
            st.write("The loan is granted")
        else:
            st.write("The loan is not granted")     
...
clients = random.choices(data_train2.SK_ID_CURR.tolist(),k = 100)
...
with col3:
    st.subheader("Gender")
    Gender = data_train2[data_train2["SK_ID_CURR"] == client_id]["CODE_GENDER"].values[0]
    if Gender >0:
        st.write("Male")
    else:
        st.write("Female")
